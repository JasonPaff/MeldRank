import type { RoomCoreState, SeatAssignment } from './types';
...
export function withSeat(seats: readonly SeatAssignment[], seatIndex: number, connectionId: string): SeatAssignment[] {
  const next: SeatAssignment[] = [...seats, { seatIndex, connectionId, token: stubSeatToken(seatIndex, connectionId) }];
  return next.sort((a, b) => a.seatIndex - b.seatIndex);
}
