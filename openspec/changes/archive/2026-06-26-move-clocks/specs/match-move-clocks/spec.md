...
The room SHALL count consecutive or accumulated clock timeouts per seat, and when a seat crosses a configured timeout threshold in a ranked room, the room SHALL emit an abandonment signal effect identifying the seat. This slice SHALL only emit the signal; it SHALL NOT itself forfeit, substitute, or remove the seat (that behavior belongs to the disconnect/reconnect/abandonment slice). In casual rooms the threshold behavior MAY differ, but no abandonment action is taken in this slice.
...
#### Scenario: Signal does not act on the seat

- **WHEN** the abandonment signal is emitted
- **THEN** the seat remains seated and play continues
- **AND** no forfeit or substitution is performed in this slice
