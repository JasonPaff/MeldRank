...
The engine foundation gave `@meldrank/engine` its domain model (`Card`, `Deck`, `Seat`, `Hand`, `Bid`/`Contract`, …) and a _structure-only_ hand-lifecycle machine (`LifecyclePhase` union, legal-transition table, `resolveActivePath`). Nothing drives that machine yet. This change adds the first two phase drivers — Dealer and AuctionManager — and, with them, the runtime spine they hang on: a pure `reduce(state, event)` state container.
...
- The CSPRNG, commit–reveal, seed assembly, and per-seat view _filtering_ — owned by Match Runtime / Anti-Cheat. The engine only exposes the injected-`rng` seam and structures state so filtering is trivial.
...
- **[The `State`/`Event` shape ossifies early and later phases need to reshape it]** → This change deliberately wires only `Dealing → Auction` but designs `Event` as the _full_ locked union and `State` with the public/private split all phases need, so later changes _extend_ (add phase sub-state, implement a guard branch) rather than reshape. The reducer is data-driven off the existing transition table, so adding a phase is additive.
