...
`reduce` SHALL validate each event against the current lifecycle phase and reject any event not legal in that phase, returning the state unchanged (or a typed rejection) without mutation. This change drives only the `Dealing → Auction` slice; events belonging to later phases (`declareTrump`, `playCard`) SHALL be accepted by the type but rejected by the guard until their phases are implemented.
...
When an event concludes a phase, `reduce` SHALL advance the phase marker to the next phase using the foundation's legal-transition table and the variant's active path (`resolveActivePath`), skipping bracketed phases the variant disables. `reduce` SHALL NOT advance along a transition the table reports as illegal.

#### Scenario: A concluded auction advances to the variant's next active phase

- **WHEN** the auction concludes with a winning bid for `SINGLE_DECK_PARTNERS`
- **THEN** the winning `Bid` is recorded on the state and the phase marker advances to `DeclareTrump` (Partners skips `WidowReveal`); driving `DeclareTrump` is deferred to a later change

#### Scenario: Bracketed phases are skipped per variant

- **WHEN** the auction concludes for `SINGLE_DECK_CUTTHROAT`
- **THEN** the phase marker advances to `WidowReveal` (enabled for Cutthroat) rather than directly to `DeclareTrump`
...
`State` SHALL separate per-seat private data (each seat's `Hand`, the unrevealed widow) from public data (phase, turn, auction standing, the recorded winning `Bid`) so the Match Service can mechanically derive a per-seat filtered view. The engine SHALL structure the state for filtering; performing the filtering is Match Runtime's responsibility ("Match Runtime" §3).

#### Scenario: Private and public regions are distinguishable

- **WHEN** a `State` mid-hand is inspected
- **THEN** each seat's hand and the unrevealed widow are addressable as private, while the phase, whose-turn, and auction bids are addressable as public
