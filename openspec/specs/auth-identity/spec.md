...
Defines the single identity edge for the API: authenticating each caller from a Clerk Bearer session, resolving (and lazily creating) the internal player id, caching that mapping, syncing player rows from Clerk webhooks, and keeping the match service outside the Clerk path by carrying identity only through the seat ticket.
