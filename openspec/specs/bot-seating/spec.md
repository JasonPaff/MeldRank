# bot-seating Specification

## Purpose

Defines how an in-process bot occupies a real seat in the room: a first-class `SeatAssignment` in the pure `RoomCore` that counts toward fullness and passes the same authority guards as a human, the prohibition on seating bots in ranked rooms, the Colyseus adapter driving a bot seat's turn through the bot brain on the seat's synthetic connection, a humanized think delay on the room clock, and reuse of the same bot driver for casual disconnect-takeover seats.
