...
Defines the isomorphic, browser-safe contract surface exported from `@meldrank/shared` — the binding Zod schemas for every tRPC procedure, the room-spawn request/response pair, the seat-ticket payload and its server-only sign/verify helper, the ephemeral casual-table record shape, and the shared pagination envelope and error taxonomy.
