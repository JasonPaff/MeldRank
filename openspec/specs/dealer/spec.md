...
Defines the Dealer in `@meldrank/engine` — a pure, deterministic function that distributes a built deck into one `Hand` per seat plus a widow, per "Game Engine — Abstract Model" §5. It owns the shuffle (Fisher–Yates over an injected `rng`) and the deal slice, while entropy, CSPRNG keying, and commit–reveal remain the responsibility of Match Runtime / Anti-Cheat.
