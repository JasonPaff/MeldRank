...
Defines the provably-fair shuffle fairness module — a pure, isomorphic core that turns multi-party entropy into a verifiable, replay-sufficient deal. It binds the server to a `serverSeed` via a commit before the deal, assembles a single `seed` from the `serverSeed`, a per-hand nonce, and per-seat `clientSeed` contributions, derives a full-width `Rng` that feeds the engine Dealer, and provides a post-hand reveal bundle plus a `verify` function so any third party can reproduce and audit the deal. Entropy contribution, drop-out fallback, and verification are owned here; the shuffle/deal slice itself remains in `@meldrank/engine` ("Anti-Cheat" §2).
