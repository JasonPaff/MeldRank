...
Defines the pure bury-eligibility and bury-composition rules in `@meldrank/engine` for bury-enabled variants (e.g. Single-Deck Cutthroat / Auction Pinochle). It establishes which of the bidder's cards may be buried under the variant's restrictions and what makes a proposed bury legal, so the hand-state container can guard the `Bury` phase deterministically.
