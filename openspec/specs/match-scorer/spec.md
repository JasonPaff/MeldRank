...
Defines the pure `MatchScorer` in `@meldrank/engine` that decides whether a match has ended and, when it has, produces the final standings and rating basis, per "Game Engine — Abstract Model" §5 and "Single-Deck Partners" §9. It reads the running score pad, the just-finished hand result, and the per-side hands-made-as-bidder tiebreak counter to evaluate the variant's match-end condition and place each side.
