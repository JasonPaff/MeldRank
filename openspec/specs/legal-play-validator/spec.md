...
Defines the pure `LegalPlayValidator` in `@meldrank/engine` that, given a seat's hand, the in-progress trick, the declared trump, and the variant's `trickRules`, computes the subset of the hand the seat may legally play, per "Single-Deck Partners" §7. It enforces the follow-suit / trump-when-void obligation cascade and the optional strict must-beat (must-head and over-trump) rule, gated by variant flags so a single function serves variants that relax an obligation.
