...
Defines the pure `HandScorer(melds, captured, contract, buriedCounters, variant) → HandResult` function in `@meldrank/engine`, per "Game Engine — Abstract Model" §5 and "Single-Deck Partners" §8. It folds per-seat melds and captured counters into per-side results, applies the meld-needs-a-trick gate, evaluates the bidding side's contract for made/set with the configured set penalty, and honors the variant's scoring mode (all-sides-score vs bidder-vs-bid).
