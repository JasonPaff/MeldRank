...
Defines how a completed match is accumulated, assembled into a self-describing durable record, written transactionally to Postgres, and announced as a status-only result event — driving the room's transition to `Persisted` only after the durable write confirms.
