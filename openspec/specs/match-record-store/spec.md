# match-record-store Specification

## Purpose

Define the durable schema for recording completed and aborted matches: players and bot profiles, the match envelope with a self-describing variant snapshot, participant seats, scorecard hands and per-side lines, opaque replay storage, and append-only abandon events. This capability owns the domain table definitions that live in the Drizzle schema home, and ships a pure as-scored hand projector that maps plain hand results to insert rows without depending on the engine or a database.
