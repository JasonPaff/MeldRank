...
Defines how `@meldrank/engine` deterministically reveals the widow for widow-enabled variants when the auction concludes: moving the widow into the contract winner's hand, recording the revealed widow publicly, and passing through the `WidowReveal` phase, per "Match Runtime" §3/§4 and the canonical exposed-widow rule.
