...
Defines the pure `MeldDetector` function in `@meldrank/engine` that computes a seat's maximum-scoring legal meld set from its hand, the declared trump, and the standard meld table, per "Game Engine — Abstract Model" §5. Meld is engine-computed (not player-chosen): the detector recognizes every Class A, B, and C meld present, enforces cross-class reuse / within-class exclusivity rules, and applies double-meld bonuses, returning the seat's scored melds and total.
