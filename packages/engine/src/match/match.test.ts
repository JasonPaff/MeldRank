...
describe('MatchScorer — match-end (target-score §9 count-out)', () => {
  it('continues while no side has reached the target', () => {
    const result = MatchScorer(pad({ 0: 250, 1: 71 }, 1), handResult(0, true), {}, targetScore(1500));
  ...
  it('counts the bidding side out first when both sides cross the target', () => {
    ...
    const result = MatchScorer(pad({ 0: 1500, 1: 1600 }, 7), handResult(0, true), {}, targetScore(1500));
  ...
  it('lets the non-bidding side win when the bidder did not count out', () => {
    ...
    const result = MatchScorer(pad({ 0: 900, 1: 1500 }, 7), handResult(0, false), {}, targetScore(1500));
  ...
  it('continues when the bidder is set at/above target and no opponent reached (residual)', () => {
    ...
    const result = MatchScorer(pad({ 0: 1500, 1: 200 }, 7), handResult(0, false), {}, targetScore(1500));
    expect(result.complete).toBe(false);
  });
});
...
describe('MatchScorer — standings, placement, and tiebreak', () => {
  it('orders sides by cumulative descending (fixed-deals)', () => {
    const result = MatchScorer(pad({ 0: 100, 1: 200, 2: 300 }, 9), handResult(0, true), {}, SINGLE_DECK_CUTTHROAT);
  ...
  it('breaks a cumulative tie by most hands made as bidder', () => {
    ...
    const result = MatchScorer(pad({ 0: 500, 1: 500, 2: 300 }, 9), handResult(0, true), { 0: 2, 1: 1 }, SINGLE_DECK_CUTTHROAT);
  ...
  it('shares a placement for fully-tied sides and skips the next placement', () => {
    ...
    const result = MatchScorer(pad({ 0: 500, 1: 500, 2: 300 }, 9), handResult(0, true), { 0: 1, 1: 1 }, SINGLE_DECK_CUTTHROAT);
...
describe('MatchScorer — rating basis outcomes', () => {
  it('marks exactly one winner under team-win-loss (Partners)', () => {
    const result = MatchScorer(pad({ 0: 1550, 1: 1200 }, 8), handResult(0, true), {}, SINGLE_DECK_PARTNERS);
  ...
  it('carries ordinal placements under individual-placement (Cutthroat)', () => {
    const result = MatchScorer(pad({ 0: 100, 1: 200, 2: 300 }, 9), handResult(2, true), {}, SINGLE_DECK_CUTTHROAT);
